---
title: "Analysing the outputs of ChromEvol for sensitivity analyses"
...
summary <- read_delim(here("chromEvol/chromEvol_out_scaled_noBrookesia/models_summary.txt"), delim = "\t")
# Add AIC weights
summary <-
  summary %>%
...
# containing them
results <- read.ce(here(paste0("chromEvol/chromEvol_out_scaled_noBrookesia/", best_model)))
```
...
```{r, message = FALSE}
# Read in the model summary data
summary <- read_delim(here("chromEvol/chromEvol_out_scaled_species/models_summary.txt"), delim = "\t")
# Add AIC weights
summary <-
  summary %>%
  mutate(AICw = round(aic.w(AIC), digits = 3))
...
# Read in the results by specifying the path to the folder
# containing them
results <- read.ce(here(paste0("chromEvol/chromEvol_out_scaled_species/", best_model)))
```
