...
4. Look at all pairs of genera. 

```{r}
mydata_genus <-
  mydata %>% 
  mutate(dup = duplicated(mydata$genus)) %>%
  filter(dup != TRUE)

# Remove genus from the tree that are not in mydata_genus
remove <- setdiff(mytree$tip.label, mydata_genus$tips)
genustree <- drop.tip(mytree, remove)
plot(genustree)
```

```{r}
# Get genus pairs
all.pairs_genus <- expand.grid(tip1 = mydata_genus$tips,
	                       tip2 = mydata_genus$tips)

# Combine with the chromosome data...
all.pairs_genus  <- left_join(all.pairs_genus, mydata_genus, by = c("tip1" = "tips"))
all.pairs_genus  <- left_join(all.pairs_genus, mydata_genus, by = c("tip2" = "tips"))

# Find cophenetic distances for whole tree
phylo_matrix_genus <- cophenetic.phylo(genustree)

# Flatten matrix
phylo.dist.all_genus <- 
  data.frame(expand.grid(tip1 = dimnames(phylo_matrix_genus)[[1]],
	                       tip2 = dimnames(phylo_matrix_genus)[[2]]),
             phylodist = c(phylo_matrix_genus))

# Add to pairs data
# warns about adding vectors and factors - this is fine here
pairs.data_genus <- 
  left_join(phylo.dist.all_genus, all.pairs_genus, by = c("tip1","tip2")) %>% 
  filter(phylodist > 0)
...
And plot...
```{r}
ggplot(pairs.data2_genus, aes(x = phylodist, y = haploid_diff)) +
  geom_point(alpha = 0.5) +
  theme_bw()
  
# Or hex plot
#ggplot(pairs.data2_genus, aes(x = phylodist, y = haploid_diff)) +
#  geom_hex(alpha = 0.5, bins = 10) +
#  theme_bw()
```
