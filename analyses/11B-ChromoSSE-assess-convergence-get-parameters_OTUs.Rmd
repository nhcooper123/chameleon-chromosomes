...
trace_quant <- readTrace(path = here("ChromoSSE/OTUs/output/ChromoSSE_exclude_n18_model.log"), burnin = 0.1)
```
...
```{r}
summarizeTrace(trace = trace_quant, vars =  c("gamma","delta"))

```
Plot traces for variables
```{r}
plotTrace(trace = trace_quant, vars = c("gamma","delta"))[[1]]
```