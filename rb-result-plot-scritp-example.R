# https://github.com/wf8/ChromoSSE/blob/master/plot_results.R
...
#
# I had to modify this as it doesn't work anymore coz revgadgets has changed!!!

library(RevGadgets)
library(ggplot2)
library(ggtree)

x <- "ChromoSSE-master/examples/aristolochia/data/aristolochia_ancestral_states.tree"

# Create anc states
t = processAncStates(x)

# plot the ancestral states tree generated by RevBayes
p = plotAncStatesMAP(t, size=0.4, alpha=.3)
...
p = p + geom_segment(x=arrow_xend-0.4, y=arrow_yend+0.6, xend=arrow_xend, yend=arrow_yend, 
                     linewidth=0.5, color="grey35", arrow=arrow(length=unit(0.2, "cm"), type="open", angle=30), inherit.aes=FALSE)
